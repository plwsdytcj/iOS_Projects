...
extension Net {
    static func dataTaskWithRequest(_ request: NSURLRequest, _ viewController: UIViewController, loadingViewType: LoadingViewType = .Controller, loadingTitle: String = "Connecting", successCode: Int = 200, errorHandler: ((UIAlertAction) -> Void)? = nil, backgroundSuccessHandler: ((AnyObject?) -> Void)? = nil, mainSuccessHandler: @escaping (AnyObject?) -> Void) -> URLSessionDataTask {
        ...
        let dataTask = API.dataTaskWithRequest(request) { JSONObject, statusCode, error in
            ...
            if let backgroundSuccessHandler = backgroundSuccessHandler, statusCode == successCode {
